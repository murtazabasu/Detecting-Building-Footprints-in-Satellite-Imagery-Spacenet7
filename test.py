...
def create_dist_map(rasterSrc, vectorSrc, npDistFileName='', 
    ...
    geoTrans, poly, ulX, ulY, lrX, lrY = getRasterExtent(srcRas_ds)
    transform_WGS84_To_UTM, transform_UTM_To_WGS84, utm_cs \
                                                = createUTMTransform(poly)
...
def main():    
    ...
    imDir = os.path.join(spacenet_explore_dir, '3band')
    vecDir = os.path.join(spacenet_explore_dir, 'vectorData')
    ...
    # make dirs
    for p in [imDir_out, vecDir, coords_demo_dir, maskDir, maskDir_vis, mask_demo_dir,
              distDir, dist_demo_dir, all_demo_dir]:
        if not os.path.exists(p):
            os.mkdir(p)

    # get input images and copy to working directory
    rasterList = os.listdir(imDir)
    print(rasterList)
    ...
    for i,rasterSrc in enumerate(rasterList):
        ...
        input_image = plt.imread(imDir + "\\" + rasterSrc) # cv2.imread(rasterSrc, 1)
        
         # get name root
        name_root0 = rasterSrc.split('.')[0]
        # remove 3band or 8band prefix
        vectorSrc = os.path.join(vecDir, name_root0 + '_Buildings.geojson')
        maskSrc = os.path.join(maskDir, name_root0 + '.tif')
        
        ####################################################
        # pixel coords and ground truth patches
        pixel_coords, latlon_coords = \
            geojson_to_pixel_arr(imDir + "\\" + rasterSrc, vectorSrc, 
                                                      pixel_ints=True,
                                                      verbose=False)
        pixel_coords_list.append(pixel_coords)
       
        plot_name = os.path.join(coords_demo_dir, name_root0 + '.png')
